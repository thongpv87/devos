...
with lib;
{
  ...
  config = mkIf (cfg.enable) (mkMerge [
...

    {
      ...
      boot.kernelParams =
        if (cfg.cpuScaling == "intel_pstate") then [ ]
        else if (cfg.cpuScaling == "intel_cpufreq") then [ "intel_pstate=passive" ]
        else if (cfg.cpuScaling == "acpi_cpufreq") then [ "intel_pstate=disable" ]
        else [ ];
      ...
      services.tlp = {
        ...
        settings =
          let
            governor = (if (cfg.cpuScaling != "schedutil") then "powersave" else "schedutil");
