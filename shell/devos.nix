...
let
  inherit (pkgs)
    agenix alejandra cachix editorconfig-checker nixUnstable nodePackages shfmt
    treefmt nvfetcher-bin nixos-generators;
...
in {
  ...
  packages = [ alejandra nodePackages.prettier shfmt editorconfig-checker ];
  ...
  commands = [
    ...
    (devos agenix)
    {
      ...
      command =
        "cd $PRJ_ROOT/pkgs; ${nvfetcher-bin}/bin/nvfetcher -c ./sources.toml $@";
    }
